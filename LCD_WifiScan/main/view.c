...
static void wifi_screen_create(module_t*self){
    ...
    // lv_obj_set_style_text_font(wifi_label,&lv_font,0);
    ...
    lv_label_set_text(wifi_tip,"WIFI未连接");
    lv_obj_set_style_text_font(wifi_tip,&lv_font_chinese,0);
    lv_obj_set_style_text_font(mod->wifi_info,&lv_font_chinese,0);
...
static void ui_demo1(module_t *self){
    view_module_t *mod = (view_module_t *)self;
    lvgl_port_lock(0); 
    ui_init();
    ...
    block_init();
    /* 创建各区块的内容 */
    top_block_create(self);
    bottom_block_create(self);
    main_block_create(self);
    /* 以上界面初始化完毕  */
    /* 下面开始订阅事件 */
